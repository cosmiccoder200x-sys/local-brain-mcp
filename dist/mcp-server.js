...
import { MAX_LESSON_LENGTH, DUPLICATE_SIMILARITY_THRESHOLD, MAX_RESPONSE_TOKENS, } from "./config.js";
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        if (name === "brain_learn") {
            ...
            try {
                ...
                debugLog("mcp", "Failed to get git info for brain_learn: %s", e instanceof Error ? e.message : String(e));
            }
            const projectId = getProjectId();
            const packageScope = derivePackageScope(file_path);
            const embedding = embed(lesson);
            // Duplicate check & smart merge
            const dup = findDuplicateMemory(db, embedding, lesson, file_path, DUPLICATE_SIMILARITY_THRESHOLD);
        ...
        // ── brain_prune ───────────────────────────────────────────────────────────
        if (name === "brain_prune") {
            const status = (args?.status ?? "stale");
