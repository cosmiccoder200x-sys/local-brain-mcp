...
let explicitColorOverride: boolean | null = null;
...
function wrap(open: string, close: string = RESET): (text: string | number) => string {
  return (text: string | number) => {
    if (!isColorSupported() || text === undefined || text === null) return String(text ?? '');
...
export function rgb(r: number, g: number, b: number, text: string | number): string {
  ...
  if (!isColorSupported() || !str) return str;
  return `${ESC}38;2;${Math.round(r)};${Math.round(g)};${Math.round(b)}m${str}${RESET}`;
}

// ─── Gradient System (Purple → Blue → Cyan) ──────────────────────────────────
...
const BRAND_STOPS: RgbColor[] = [
  { r: 168, g: 85,  b: 247 }, // #A855F7 Purple
  { r: 59,  g: 130, b: 246 }, // #3B82F6 Blue
  { r: 6,   g: 182, b: 212 }, // #06B6D4 Cyan
];

function interpolateRgb(c1: RgbColor, c2: RgbColor, factor: number): RgbColor {
  return {
...
function getGradientColor(fraction: number, stops: RgbColor[] = BRAND_STOPS): RgbColor {
 ...
 */
export function gradient(text: string | number, stops: RgbColor[] = BRAND_STOPS): string {
  const str = String(text);
  if (!isColorSupported() || !str) return str;

  // Strip ANSI sequences when computing string length to avoid distortion
  const clean = str.replace(/\x1b\[[0-9;]*m/g, '');
  if (clean.length === 0) return str;
...
export function brand(text: string | number): string {
  return gradient(text);
}

export function success(text: string | number): string {
  return green(text);
}

export function warning(text: string | number): string {
  return yellow(text);
}

export function error(text: string | number): string {
  return red(text);
}

export function muted(text: string | number): string {
  return gray(text);
}
...
export function heading(text: string | number): string {
  const str = String(text);
  if (!isColorSupported()) return str;
  return `${ESC}1m${ESC}96m${str}${RESET}`;
}

export function label(name: string, width: number = 14): string {
  const padded = name.padEnd(width);
  return isColorSupported() ? `${ESC}90m${padded}${RESET}` : padded;
}

export function keyVal(name: string, val: string | number, labelWidth: number = 14): string {
...
export function badge(text: string, type: 'brand' | 'success' | 'warning' | 'error' | 'muted' = 'brand'): string {
 ...
 */
export function compactMark(): string {
 ...
 */
export function card(lines: string[], innerWidth: number = 52): string {
  const topBorder    = `╭${'─'.repeat(innerWidth)}╮`;
  const bottomBorder = `╰${'─'.repeat(innerWidth)}╯`;
  const emptyLine    = `│${' '.repeat(innerWidth)}│`;

  const border = isColorSupported() ? gray : (s: string) => s;
 ...
 */
export function headerBanner(): string {
  const innerWidth = 52;
