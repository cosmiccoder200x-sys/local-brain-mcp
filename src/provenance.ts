 ...
 */
export function normalizeAgentId(raw?: string | null): AgentId {
...
export function getProjectId(repoRoot?: string): string {
  ...
  try {
    ...
    debugLog(
      "provenance",
      "Failed to get git remote URL: %s",
 ...
 */
export function buildProvenanceNote(note: ProvenanceNote): string {
  const parts: string[] = [];

  parts.push(`Agent: ${note.agent}`);
  parts.push(`Source: ${note.source}`);
  if (note.project_id) parts.push(`Project: ${note.project_id}`);
  if (note.branch) parts.push(`Branch: ${note.branch}`);
  if (note.commit) parts.push(`Commit: ${note.commit.slice(0, 7)}`);
  if (note.files.length > 0) parts.push(`Files: ${note.files.slice(0, 3).join(", ")}`);
