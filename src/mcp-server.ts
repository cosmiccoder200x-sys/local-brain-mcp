...
function makeErrorResponse(message: string, hint?: string): CallToolResult {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
      ...
      if (args?.category !== undefined) {
        ...
        if (!VALID_CATEGORIES.has(cat)) {
          return makeErrorResponse(
            ...
            `Supported categories are: ${Array.from(VALID_CATEGORIES).join(', ')}.`
          );
        }
    ...
    if (name === 'brain_learn') {
      ...
      let category: MemoryCategory = 'manual';
      if (args?.category !== undefined) {
        const cat = String(args.category).toLowerCase();
        if (!VALID_CATEGORIES.has(cat)) {
          return makeErrorResponse(
    ...
    // ── 4. brain_prune ────────────────────────────────────────────────────────
    if (name === 'brain_prune') {
      let status: MemoryStatus | 'all' = 'stale';
      if (args?.status !== undefined) {
        const s = String(args.status).toLowerCase();
        if (!VALID_PRUNE_STATUSES.has(s)) {
          return makeErrorResponse(
            `Invalid status "${args.status}".`,
            `Supported values are: ${Array.from(VALID_PRUNE_STATUSES).join(', ')}.`
          );
        }
        status = s as MemoryStatus | 'all';
      }
