    ...
    for (const editor of EDITOR_TARGETS) {
      ...
      if (existsSync(editor.configPath)) {
        try {
          ...
          debugLog(
            "cli",
            "Failed to parse editor config %s: %s",
            editor.configPath,
      ...
      console.log(
        `  ${success("✓")} ${highlight(editor.name)} — config updated at ${muted(editor.configPath)}`
      );
      detected++;
    }
  ...
  .action(
    async (opts: {
      ...
      if (!opts.quiet) {
        ...
        console.log(
          `     ${label("Errors", 12)} ${result.errors > 0 ? error(result.errors) : muted(result.errors)}\n`
        );
      }
    }
  );
...
program
  ...
  .action(async (lesson, opts) => {
    ...
    const importance_level: ImportanceLevel =
      (opts.importanceLevel as ImportanceLevel) || detectImportanceLevel(lesson);
...
program
  ...
  .action((opts) => {
    ...
    const conditions: string[] = status === "all" ? [] : ["status = ?"];
    ...
    for (const r of rows) {
      ...
      const val =
        (r.validation_count ?? 0) > 0 ? ` ${success(`[val: ${r.validation_count}×]`)}` : "";
      ...
      console.log(
        `  • ${highlight(`#${r.id}`)} ${agTag} ${catTag}${val}${cFlag}: ${r.summary.slice(0, 120)}`
      );
    }
    console.log("");
  });
...
program
  ...
  .action((filePath, opts) => {
    ...
    for (const m of memories) {
      ...
      const valTag =
        (m.validation_count ?? 0) > 0 ? ` ${success(`[validated: ${m.validation_count}×]`)}` : "";
      const commitTag = m.commit_hash ? ` ${muted(`@ ${m.commit_hash.slice(0, 7)}`)}` : "";
      console.log(
        `  • ${highlight(`#${m.id}`)} ${muted(`[${m.category}]`)}${statusTag}${supersededTag}${agentTag}${valTag}${commitTag}: ${m.summary.slice(0, 180)}`
      );
    }
    console.log("");
  });
...
program
  ...
  .action(() => {
    ...
    console.log(
      `  ${label("DB file exists", 18)} ${existsSync(dbPath) ? success("✓ YES") : muted("ℹ NO (will be created on first ingest)")}`
    );
    ...
    for (const editor of EDITOR_TARGETS) {
      if (existsSync(editor.configPath)) {
        try {
          ...
          console.log(
            `    • ${editor.name.padEnd(18)}: ${configured ? success("✓ CONFIGURED") : warning("▲ FILE EXISTS, MCP NOT LINKED")}`
          );
        } catch (e) {
          debugLog(
            "cli",
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      const recentRows = db
        .prepare(
          `SELECT summary, agent, created_at FROM memories WHERE status = 'active' ORDER BY created_at DESC LIMIT 3`
        )
        .all() as { summary: string; agent: string; created_at: string }[];
      ...
      debugLog(
        "cli",
        "Failed to fetch recent knowledge: %s",
        e instanceof Error ? e.message : String(e)
      );
    }
  });
...
program
  ...
  .action(async (opts: { repo: string; status: string; invalidate?: boolean }) => {
    ...
    if (opts.invalidate) {
      ...
      console.log(
        `     ${label("Memories stalified", 20)} ${inv.stalifiedCount > 0 ? warning(inv.stalifiedCount) : muted(inv.stalifiedCount)}`
      );
